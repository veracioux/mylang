...
class if_(Object, FunctionAsClass):
    ...
    def _m_classcall_(cls, args, /):
        ...
        if condition is None:
            ...
            modified_statement_list = StatementList.from_iterable(
                (stud_args + Args(statement))
                for statement in statement_list
            )
            for i, orig_statement in enumerate(statement_list):
                orig_statement: Args
                assert isinstance(orig_statement[-1], StatementList), "The last argument in each statement of an if-else block must be a StatementList"
                if isinstance(orig_statement[0], String) and orig_statement[0] == String("else"):
                    ...
                    assert orig_statement.is_positional_only() and len(orig_statement) == 2, "condition statement must have exactly 2 arguments"
            ...
            current_stack_frame.get().lexical_scope.custom_data[_Symbols.CURRENT_IF_BLOCK_DATA] = cls.__IfBlockData(modified_statement_list)

            value = modified_statement_list.execute()
            return value
        # Simple if statement with single condition
        elif condition:
            if_block_data = current_stack_frame.get().lexical_scope.custom_data.get(_Symbols.CURRENT_IF_BLOCK_DATA, None)
    ...
    @classmethod
    def _update_if_block_data(cls, ):
        """Update the data regarding the current if-else block in the current stack frame."""
        current_stack_frame.get().lexical_scope.custom_data.setdefault(_Symbols.CURRENT_IF_BLOCK_DATA, cls.__IfBlockData())
